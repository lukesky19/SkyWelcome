...
/**
 * This class creates a gui to allow the selection of a custom server change message.
 */
public class ServerChangeGUI extends MessageSelectorGUI {
    /**
     * Constructor
     ...
     * @param serverChangeMessages The {@link List} of {@link MessageConfig}.
     */
    public ServerChangeGUI(
        ...
        super(skyWelcome, guiManager, player, settingsManager, playerDataManager, messageBroker, hookManager, changeGUIConfigManager.getConfiguration(), playerData, serverChangeMessages);
    }

    @Override
    public @NonNull String getPlayerMessage() {
        return playerData.getServerChangeMessage();
    }

    @Override
    protected void updatePlayerMessage(@NonNull String message) {
        playerData.setServerChangeMessage(message);
