 ...
 */
public class QuitGUI extends MessageSelectorGUI {
    /**
     * Constructor
    ...
    public QuitGUI(
        ...
        super(skyWelcome, guiManager, player, settingsManager, playerDataManager, messageBroker, hookManager, quitGUIConfigManager.getConfiguration(), playerData, joinMessages);
    }

    @Override
    public @NonNull String getPlayerMessage() {
        return playerData.getLeaveMessage();
    }

    @Override
    protected void updatePlayerMessage(@NonNull String message) {
        playerData.setLeaveMessage(message);
