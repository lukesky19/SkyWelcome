...
/**
 * This class is used to create the toggle motd command argument.
 */
public class ToggleMotDCommand {
    ...
    /**
     * Default Constructor.
     * You should use {@link #ToggleMotDCommand(SkyWelcomePaper, SettingsManager, LocaleManager, PlayerDataManager, MessageBroker)} instead.
     * @deprecated You should use {@link #ToggleMotDCommand(SkyWelcomePaper, SettingsManager, LocaleManager, PlayerDataManager, MessageBroker)} instead.
     * @throws RuntimeException if used.
     */
    @Deprecated
    public ToggleMotDCommand() {
    ...
    /**
     * Creates the {@link LiteralCommandNode} of type {@link CommandSourceStack} for the motd command argument.
     * @return A {@link LiteralCommandNode} of type {@link CommandSourceStack} for the motd command argument.
     */
    public @NonNull LiteralCommandNode<CommandSourceStack> createCommand() {
        return Commands.literal("motd")
                .requires(ctx -> ctx.getSender().hasPermission("skywelcome.commands.skywelcome.toggle.motd") && ctx.getSender() instanceof Player)
                .executes(ctx -> {
                    ...
                    if(playerData == null) {
                        player.sendMessage(AdventureUtility.deserialize(locale.prefix() + "<red>Unable to toggle the sending of the motd message due to invalid player data.</red>"));
                        logger.warn(AdventureUtility.deserialize("Unable to toggle the sending the motd message for player " + player.getName() + " due to invalid player data."));
                        return 0;
                    }
                    ...
                    if(playerData.isSendMotd()) {
                        player.sendMessage(PaperAdventureUtility.deserialize(player, locale.prefix() + locale.motdEnabled()));
                    } else {
                        player.sendMessage(PaperAdventureUtility.deserialize(player, locale.prefix() + locale.motdDisabled()));
                    }
