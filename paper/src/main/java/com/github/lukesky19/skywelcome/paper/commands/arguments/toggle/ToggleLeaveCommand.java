...
/**
 * This class is used to create the toggle leave/quit command argument.
 */
public class ToggleLeaveCommand {
    ...
    /**
     * Default Constructor.
     * You should use {@link #ToggleLeaveCommand(SkyWelcomePaper, SettingsManager, LocaleManager, PlayerDataManager, MessageBroker)} instead.
     * @deprecated You should use {@link #ToggleLeaveCommand(SkyWelcomePaper, SettingsManager, LocaleManager, PlayerDataManager, MessageBroker)} instead.
     * @throws RuntimeException if used.
     */
    @Deprecated
    public ToggleLeaveCommand() {
    ...
    public @NonNull LiteralCommandNode<CommandSourceStack> createCommand(@NonNull String command) {
        return Commands.literal(command)
                .requires(ctx -> ctx.getSender().hasPermission("skywelcome.commands.skywelcome.toggle." + command) && ctx.getSender() instanceof Player)
                .executes(ctx -> {
                    ...
                    if(playerData == null) {
                        player.sendMessage(AdventureUtility.deserialize(locale.prefix() + "<red>Unable to toggle the sending of your leave message due to invalid player data.</red>"));
                        logger.warn(AdventureUtility.deserialize("Unable to toggle the sending of player " + player.getName() + "'s leave message due to invalid player data."));
                        return 0;
                    }
                    ...
                    if(playerData.isSendLeave()) {
                        player.sendMessage(PaperAdventureUtility.deserialize(player, locale.prefix() + locale.quitEnabled()));
                    } else {
                        player.sendMessage(PaperAdventureUtility.deserialize(player, locale.prefix() + locale.quitDisabled()));
                    }
