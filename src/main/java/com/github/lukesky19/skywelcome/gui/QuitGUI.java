...
public class QuitGUI extends ChestGUI {
    ...
    public QuitGUI(
        ...
        this.guiConfig = guiConfigManager.getQuitGUIConfig();
    }
    ...
    public boolean update() {
        ...
        for(GUIConfig.ButtonConfig buttonConfig : guiConfig.gui().buttons()) {
            switch(buttonConfig.buttonType()) {
                case NEXT_PAGE -> {
                    if(numOfMessagesAdded >= itemsPerPage && currentMessageKey <= (settings.quitMessages().size() - 1)) {
    ...
    /**
     * Create the items displayed for the quit messages.
    ...
    private void createMessageButtons(int itemsPerPage) {
        ...
        while(numOfMessagesAdded < itemsPerPage) {
            if(currentMessageKey >= settings.quitMessages().size() || slots.isEmpty()) return;
                ...
                if(playerData.getLeaveMessage().equals(quitMessageConfig.message())) {
                    ...
                    optionalItemStack.ifPresentOrElse(itemStack -> {
                        ...
                        guiButtonBuilder.setAction(inventoryClickEvent -> {
                            playerData.setLeaveMessage(quitMessageConfig.message());
    ...
    /**
     * Handle when a quit message button cannot be shown due to an error.
     */
